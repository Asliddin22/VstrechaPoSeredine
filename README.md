...
def pr(keys):
    for x in keys:
        print("[", end=' ')
        print('key1=', bin(x[0]), end=' ')
        print('key2=', bin(x[1]), end='')
        print("]", end=' ')
    print()
    print()
...
key1 = random.randint(0, 1023)
print ("key1 =", bin(key1))
key2 = random.randint(0, 1023)
print ("key2 =", bin(key2))
des1 = SimplifiedDES(key1)
des2 = SimplifiedDES(key2)
...
ciphertext = des2.encrypt(des1.encrypt(plaintext))
print("открытый текст:", bin(plaintext))
print("шифрованный текст:", bin(ciphertext))


for i in range(1024):
    des = SimplifiedDES(i)
    des1_ciphertext = des.encrypt(plaintext)
    if des1_ciphertext not in first_keys:
        first_keys[des1_ciphertext] = []
    first_keys[des1_ciphertext].append(i)
    des2_plaintext = des.decrypt(ciphertext)
    if des2_plaintext not in second_keys:
        second_keys[des2_plaintext] = []
    second_keys[des2_plaintext].append(i)


for i in first_keys:
    if i in second_keys:
        for x in first_keys[i]:
            for y in second_keys[i]:
                interspection.append([x,y])

#pr(interspection)
print("число возможных пар", len(interspection))
print()
first_keys.clear()
second_keys.clear()

plaintext=0
while (plaintext!=256 and len(interspection)!=1):

    ciphertext = des2.encrypt(des1.encrypt(plaintext))
    print("открытый текст:", bin(plaintext))
    print("шифрованный текст:", bin(ciphertext))
    for x in interspection:
        des = SimplifiedDES(x[0])
        des1_ciphertext = des.encrypt(plaintext)
        des = SimplifiedDES(x[1])
        des2_plaintext = des.decrypt(ciphertext)
        if des1_ciphertext == des2_plaintext:
            pairs.append(x)

    interspection = pairs.copy()
    #pr(interspection)
    print("число возможных пар", len(interspection))
    print()
